function export_rail_dynamics_baseline(j1, cut_freq, output_path)
% Export RailDyn_ModalFT output for Python migration checks.
...
if nargin < 3 || isempty(output_path)
    output_path = fullfile(tempdir, sprintf('sditt_rail_dynamics_j%g_cut%g.mat', j1, cut_freq));
end
...
save(output_path, ...
    'j1', 'cut_freq', 'ShapeFunction', 'RailBeam_Motion', ...
    'Zwy', 'Zsd', 'Zjsd', 'Dis_Rail', 'Vel_Rail', 'Acc_Rail', 'DynStatus_Rail', '-v7');
fprintf('saved %s\n', output_path);
end

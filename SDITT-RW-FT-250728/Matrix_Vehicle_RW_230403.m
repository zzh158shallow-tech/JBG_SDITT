...
function [Mlc, Klc, Clc] = Matrix_Vehicle_RW_230403(InpPar, Par_Vehicle)
...
K1z_dp = Par_Vehicle.K1z_dp;
C1z_dp = Par_Vehicle.C1z_dp;
...
while i1<8
    ...
    Klc(pos_B+2,pos_B+3) = Klc(pos_B+2,pos_B+3)-H4*K1y;
    Klc(pos_B+2,pos_B+5) = Klc(pos_B+2,pos_B+5)+((-1)^i1g4)*Ll1*K1y;
    Klc(pos_B+2,pos_W+2) = Klc(pos_B+2,pos_W+2)-K1y;
    %%% 23、28行：δφti(i = 1,2)    
    Klc(pos_B+3,pos_B+2) = Klc(pos_B+3,pos_B+2)-H4*K1y;
    Klc(pos_B+3,pos_B+3) = Klc(pos_B+3,pos_B+3)+H4*H4*K1y;
    Klc(pos_B+3,pos_B+5) = Klc(pos_B+3,pos_B+5)-((-1)^i1g4)*Ll1*H4*K1y;
    Klc(pos_B+3,pos_W+2) = Klc(pos_B+3,pos_W+2)+H4*K1y;
    %%% 25、30行：δψti(i = 1,2)     
    Klc(pos_B+5,pos_B+2) = Klc(pos_B+5,pos_B+2)+((-1)^i1g4)*Ll1*K1y;
    Klc(pos_B+5,pos_B+3) = Klc(pos_B+5,pos_B+3)-((-1)^i1g4)*Ll1*H4*K1y;
    ...
    Klc(pos_W+2,pos_B+3) = Klc(pos_W+2,pos_B+3)+H4*K1y;
    Klc(pos_W+2,pos_B+5) = Klc(pos_W+2,pos_B+5)-((-1)^i1g4)*Ll1*K1y;
    Klc(pos_W+2,pos_W+2) = Klc(pos_W+2,pos_W+2)+K1y;
    
    %%% 24、29行：δβti(i = 1,2)     
    Klc(pos_B+4,pos_B+4) = Klc(pos_B+4,pos_B+4)+H4*H4*K1x;
    Klc(pos_B+4,pos_B+5) = Klc(pos_B+4,pos_B+5)+((-1)^(i1+1+1))*Lb1*H4*K1x;
    Klc(pos_B+4,pos_W+5) = Klc(pos_B+4,pos_W+5)-((-1)^(i1+1+1))*Lb1*H4*K1x;
    %%% 25、30行：δψti(i = 1,2)      
    Klc(pos_B+5,pos_B+4) = Klc(pos_B+5,pos_B+4)+((-1)^(i1+1+1))*Lb1*H4*K1x;
    ...
    Klc(pos_W+5,pos_B+4) = Klc(pos_W+5,pos_B+4)-((-1)^(i1+1+1))*Lb1*H4*K1x;
...
for i1 = 1:1:2
    for i2 = 1:1:2
        for i3 = 1:1:2
            ...
            % 36~43列：zp (i=1~8)
            Klc(pos_P, pos_P) = Klc(pos_P, pos_P) + K1z_dp;
            Klc(pos_W+1, pos_P) = Klc(pos_W+1, pos_P) - K1z_dp;
            Klc(pos_W+3, pos_P) = Klc(pos_W+3, pos_P) - ((-1)^i3)*Lb1*K1z_dp;
            
            % 1, 6, 11, 16列：zw (i=1~4)
            Klc(pos_P, pos_W+1) = Klc(pos_P, pos_W+1) - K1z_dp;
            Klc(pos_W+1, pos_W+1) = Klc(pos_W+1, pos_W+1) + K1z_dp;
            Klc(pos_W+3, pos_W+1) = Klc(pos_W+3, pos_W+1) + ((-1)^i3)*Lb1*K1z_dp;
            
            % 3, 8, 13, 18列：φw (i=1~4)
            Klc(pos_P, pos_W+3) = Klc(pos_P, pos_W+3) - ((-1)^i3)*Lb1*K1z_dp;
            Klc(pos_W+1, pos_W+3) = Klc(pos_W+1, pos_W+3) + ((-1)^i3)*Lb1*K1z_dp;
            Klc(pos_W+3, pos_W+3) = Klc(pos_W+3, pos_W+3) + Lb1*Lb1*K1z_dp;
        end
    end
end
...
for i1 = 1:1:2
    for i2 = 1:1:2
        for i3 = 1:1:2
            ...
            % 21, 26列：zt (i=1~2)
            Clc(pos_B+1, pos_B+1) = Clc(pos_B+1, pos_B+1) + C1z_dp;
            Clc(pos_P, pos_B+1) = Clc(pos_P, pos_B+1) - C1z_dp;
            Clc(pos_B+3, pos_B+1) = Clc(pos_B+3, pos_B+1) + ((-1)^i3)*Lb1*C1z_dp;
            Clc(pos_B+4, pos_B+1) = Clc(pos_B+4, pos_B+1) + ((-1)^i2)*Ll1* C1z_dp;
            
            % 36~43列：zp (i=1~8)
            Clc(pos_B+1, pos_P) = Clc(pos_B+1, pos_P) - C1z_dp;
            Clc(pos_P, pos_P) = Clc(pos_P, pos_P) + C1z_dp;
            Clc(pos_B+3, pos_P) = Clc(pos_B+3, pos_P) - ((-1)^i3)*Lb1*C1z_dp;
            Clc(pos_B+4, pos_P) = Clc(pos_B+4, pos_P) - ((-1)^i2)*Ll1* C1z_dp;
            
            % 23, 28列：φt (i=1~2)
            Clc(pos_B+1, pos_B+3) = Clc(pos_B+1, pos_B+3) + ((-1)^i3)*Lb1*C1z_dp;
            Clc(pos_P, pos_B+3) = Clc(pos_P, pos_B+3) - ((-1)^i3)*Lb1*C1z_dp;
            Clc(pos_B+3, pos_B+3) = Clc(pos_B+3, pos_B+3) + Lb1*Lb1*C1z_dp;
            Clc(pos_B+4, pos_B+3) = Clc(pos_B+4, pos_B+3) + ((-1)^i2)*Ll1*((-1)^i3)*Lb1*C1z_dp;
                       
            % 24, 29列：βt (i=1~2)
            Clc(pos_B+1, pos_B+4) = Clc(pos_B+1, pos_B+4) + ((-1)^i2)*Ll1* C1z_dp;
            Clc(pos_P, pos_B+4) = Clc(pos_P, pos_B+4) - ((-1)^i2)*Ll1* C1z_dp;
            Clc(pos_B+3, pos_B+4) = Clc(pos_B+3, pos_B+4) + ((-1)^i3)*Lb1*((-1)^i2)*Ll1*C1z_dp;
            Clc(pos_B+4, pos_B+4) = Clc(pos_B+4, pos_B+4) + Ll1*Ll1*C1z_dp;

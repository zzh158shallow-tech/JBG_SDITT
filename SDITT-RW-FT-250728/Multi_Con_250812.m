...
%  250812：清楚clear命令，加速计算

function [Mileage, Con_str, Pjc, Pjch, Pjcc, Prhx, Prhxf] = Multi_Con_250812(InpPar, Par_Vehicle, Par_Track, Par_FW, WheelPro_ProCS, ZP_Con, ZP_Dyn, j1, xlcs, ...
...
% Disp_Y  = interp1(X, InpPar.TIrr(:,3:2:end),  Mileage, Med);   % 1×N_ConPatch，奇数列：第3、5、…列 → 横向不平顺
% Disp_Z = interp1(X, InpPar.TIrr(:,2:2:end), Mileage, Med);   % 1×N_ConPatch，偶数列：第2、4、…列 → 垂向不平顺
% Vel_Y  = interp1(X, InpPar.d_TIrr(:,3:2:end),  Mileage, Med);   % 1×N_ConPatch，奇数列：第3、5、…列 → 横向不平顺
% Vel_Z = interp1(X, InpPar.d_TIrr(:,2:2:end), Mileage, Med);   % 1×N_ConPatch，偶数列：第2、4、…列 → 垂向不平顺

Fdisp_Y = griddedInterpolant(X, InpPar.TIrr(:,3:2:end), Med);       % 奇数列：第3、5、…列 → 横向不平顺
Fdisp_Z  = griddedInterpolant(X, InpPar.TIrr(:,2:2:end), Med);      % 偶数列：第2、4、…列 → 垂向不平顺
Disp_Y = Fdisp_Y(Mileage);
Disp_Z  = Fdisp_Z(Mileage);

Fvel_Y = griddedInterpolant(X, InpPar.d_TIrr(:,3:2:end), Med);       % 奇数列：第3、5、…列 → 横向不平顺
Fvel_Z  = griddedInterpolant(X, InpPar.d_TIrr(:,2:2:end), Med);      % 偶数列：第2、4、…列 → 垂向不平顺
Vel_Y = Fvel_Y(Mileage);
Vel_Z  = Fvel_Z(Mileage);

for i2 = 1:InpPar.N_ConPatch
    Dis_TIrr_temp{i2} = [0, Disp_Y(i2), Disp_Z(i2)];
    Vel_TIrr_temp{i2} = [0, Vel_Y(i2), Vel_Z(i2)];
end
...
%             Med = 'A';
            ...
            % 修正后接触信息
%             [~, Normal_Force.(T), Area_STRIPES.(T), Con_STRIPES.(T), N_Stripes.(T), Epsilon.(T)] = NF_STRIPES_230623(profile_w_Radius.(T), Profile_TrackCS.profile_r_Radius.(T), ...
%              Con_wheel_1_II.b.(T), Con_wheel_2_II.b.(T), Con_rail_1_II.b.(T), wheel_interp.(T), rail_interp.(T), ...
%              Yw_DW.(T), A_DW.(T), B_track.(T), Ver_Pen_b.(T), m.b.(T), n.b.(T), Con_A.b.(T), Con_B.b.(T), Con_r.b.(T), BGmn, Er, Vr, N_Stripes.(T), Med);
        else
...
%             temp = ZP_Con.Wheel_2.(T2){i11,num_Adv}(t1:xlcs,:);
%             bools_NaN = (temp(:,1)==0);
%             temp(bools_NaN,:) = NaN(length(find(bools_NaN)),7);
%             ConPos_w_2 = [temp(:,2:4); Con_wheel_2_II.b.(T)(i2,1:3)];
%             drta_T_All = [ZP_Dyn.drtaT(t1+1:xlcs,2); drtaT];
...
%             Vjd.(T)(i2,:) = Vel_ConPos_w_2II(end,:) * RailBeam_Motion .Win.(T2)(i11,1) * A_WS;
%             % 不考虑纵向和垂向的迁移速度
%             Vjd.(T)(i2,[1,3]) = 0;
%         else
            Vjd.(T)(i2,:) = zeros(1,3);
...
for i1=1:1:length(InpPar.Type_Side)
    ...
    for i2 = 1:1:size(Normal_Force.(T),1)
        ...
        % 231016 修改
        if size(Target_Con_RelVel_max,1)>=i11
            for kk = 1:1:length(ZP_Con.RelVel_max.(T2)(i11,:))
                if size(Target_Con_RelVel_max{i11,kk},1)>=xlcs
                    Con_RelVel_max_ori = max(Con_RelVel_max_ori, Target_Con_RelVel_max{i11,kk}(xlcs,2));
                end
            end
        end
%         % 231016 修改+250728 测试用
%         if size(Target_Con_RelVel_max,1)>=1
%             for kk = 1:1:length(ZP_Con.RelVel_max.(T2)(1,:))
%                 if size(Target_Con_RelVel_max{1,kk},1)>=xlcs
%                     Con_RelVel_max_ori = max(Con_RelVel_max_ori, Target_Con_RelVel_max{1,kk}(xlcs,2));
%                 end
%             end
%         end
        % Con_RelVel
...
%             i_Patch = Normal_Force.(T)(i2,4);
            if strcmp(InpPar.VehicleDir, 'Face')
                Win_xy = [0, 0; 28.5, 0; 40, 1; 1000, 1];                         % Face
            elseif strcmp(InpPar.VehicleDir, 'Trail')
                Win_xy = [-1000, 1; 135, 1; 150, 0; 1000, 0];         % Trail
            end
            Win = interp1(Win_xy(:,1), Win_xy(:,2), j1, 'linear');
            if strcmp(InpPar.ConDamp, 'Hu-Guo')                
                NF_Damp_Stripes_temp{i2,1} = Con_STRIPES.(T){i2,4}(:,1) .* (3*(1-InpPar.ConDamp_Coff)/2/InpPar.ConDamp_Coff.*Con_RelVel_Stripes.(T){i2,1}(:,2)) .* Con_STRIPES.(T){i2,4}(:,2);
            elseif strcmp(InpPar.ConDamp, 'Lankarani–Nikravesh')
                NF_Damp_Stripes_temp{i2,1} = Con_STRIPES.(T){i2,4}(:,1) .* (3/4*(1-InpPar.ConDamp_Coff^2).*Con_RelVel_Stripes.(T){i2,1}(:,2)) .* Con_STRIPES.(T){i2,4}(:,2);
            elseif strcmp(InpPar.ConDamp, 'Ref')
                % SIMPACK / UM
                ConStiff_Ref = 5e8;
                ConDamp_Ref = InpPar.ConDamp_Coff;
%                 ConDamp_Ref = 2e4;          % 为减小KV阻尼的影响，尝试消除 zgyg 上的残余接触点
...
%     RHLv.(T)(:,1:3) = [Vsdc.(T)(:,1)./Vgd.(T)(:,1), Vsdc.(T)(:,2)./Vgd.(T)(:,1), Vjsdc.(T)(:,3)./Vgd.(T)(:,1)];
    RHLv.(T)(:,1:3) = [Vsdc.(T)(:,1)./abs(Vgd.(T)(:,1)), Vsdc.(T)(:,2)./abs(Vgd.(T)(:,1)), Vjsdc.(T)(:,3)./abs(Vgd.(T)(:,1))];
%     RHLv.(T)(:,1:3) = [Vsdc.(T)(:,1)./abs(Vgd.(T)(:,1)), Vsdc.(T)(:,2)./abs(Vgd.(T)(:,1)), Vjsdc.(T)(:,3)./(Vgd.(T)(:,1))];
    ...
    for i2 = 1:1:size(Normal_Force.(T),1)
        ...
        Prhx_T.(T)(i2,:) = Prh.(T)(i2,:)*epxl.(T)(i2,1);
    end

function [Pxt_test, DamperNL] = NonLinear_DampingForce_CR400AF(InpPar, Par_Vehicle, xlcs, Pxt_test, DamperNL)
...
for kk = 1:1:8
    Pxt_test(pos_BG(kk)+1,1) = Pxt_test(pos_BG(kk)+1,1) - FZ_DPz_Damp(kk);
    Pxt_test(pos_Pz(kk),1) = Pxt_test(pos_Pz(kk),1) + FZ_DPz_Damp(kk);
    Pxt_test(pos_BG(kk)+3,1) = Pxt_test(pos_BG(kk)+3,1) + (-1).^(Range_i3(kk)+1).*Par_Vehicle.Lb_DPz.*FZ_DPz_Damp(kk);
    Pxt_test(pos_BG(kk)+4,1) = Pxt_test(pos_BG(kk)+4,1) + (-1).^(Range_i2(kk)+1).*Par_Vehicle.Ll_DPzt.*FZ_DPz_Damp(kk);
end
...
% 
% for kk = 1:1:4
%     Pxt_test(pos_Sn(kk),1) = Pxt_test(pos_Sn(kk),1) + FX_Sx_Damp(kk);
%     Pxt_test(pos_CB(kk)+4) = Pxt_test(pos_CB(kk)+4) - Par_Vehicle.H_cS.*FX_Sx_Damp(kk);
%     Pxt_test(pos_CB(kk)+5) = Pxt_test(pos_CB(kk)+5) + (-1).^(Range_i3(kk)) .*Par_Vehicle.Lb_S.*FX_Sx_Damp(kk);
% end

% 4*SS_Snake/BG
FX_Sx_Damp = zeros(8,1);
bools = DamperNL.Sx.Mark;
FX_Sx_Damp(bools,1) = sign(DamperNL.Sx.DampVel(bools))*(Par_Vehicle.C_Sx(1)-Par_Vehicle.C_Sx(2))*Par_Vehicle.C_Sx_V0;
...
for kk = 1:1:8
    ...
    Pxt_test(pos_Sn(kk),1) = Pxt_test(pos_Sn(kk),1) + FX_Sx_Damp(kk);
    Pxt_test(pos_CB(kk)+4) = Pxt_test(pos_CB(kk)+4) - H_cS.*FX_Sx_Damp(kk);
    Pxt_test(pos_CB(kk)+5) = Pxt_test(pos_CB(kk)+5) + (-1).^(Range_i3(kk)) .*Lb_S.*FX_Sx_Damp(kk);
end
...
% 4*SS_Snake/BG
pos_Py = pos_DOF_RV+35+16+(1:1:4)';

for kk = 1:1:4
    Pxt_test(pos_CB(kk)+2,1) = Pxt_test(pos_CB(kk)+2,1) - FY_DPy_Damp(kk,1);
    Pxt_test(pos_Py(kk),1) = Pxt_test(pos_Py(kk),1) + FY_DPy_Damp(kk,1);
    Pxt_test(pos_CB(kk)+3,1) = Pxt_test(pos_CB(kk)+3,1) + Par_Vehicle.H_cDPy.*FY_DPy_Damp(kk,1);
%     Pxt_test(pos_CB(kk)+5,1) = Pxt_test(pos_CB(kk)+5,1) - ((-1).^(Range_i1(kk)+1)).*Par_Vehicle.Ll_DPyc.*FY_DPy_Damp(kk,1);
    ...
    Pxt_test(pos_CB(kk)+5,1) = Pxt_test(pos_CB(kk)+5,1) - ((-1)^(Range_i1(kk)+1))*Ll_DPyc*FY_DPy_Damp(kk,1);
end
...
for kk = 1:1:2
    Pxt_test(pos_CB(kk)+2,1) = Pxt_test(pos_CB(kk)+2,1) - FY_STy_Stiff(kk);
    Pxt_test(pos_BG(kk)+2,1) = Pxt_test(pos_BG(kk)+2,1) + FY_STy_Stiff(kk);
    Pxt_test(pos_CB(kk)+3,1) = Pxt_test(pos_CB(kk)+3,1) + Par_Vehicle.H_cST.*FY_STy_Stiff(kk);
    Pxt_test(pos_BG(kk)+3,1) = Pxt_test(pos_BG(kk)+3,1) + Par_Vehicle.H_STt.*FY_STy_Stiff(kk);
    Pxt_test(pos_CB(kk)+5,1) = Pxt_test(pos_CB(kk)+5,1) - ((-1).^(Range_i1(kk)+1)).*Par_Vehicle.Ll_ST.*FY_STy_Stiff(kk);
end

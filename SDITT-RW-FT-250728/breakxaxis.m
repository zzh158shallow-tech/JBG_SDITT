...
%     ax = gca;
%     % 隐藏左侧y轴（竖向坐标轴）
%     ax.YAxis(1).Visible = 'off';  % 对于MATLAB R2015b及以后版本
%     % 如果需要，也可以同时清除y轴标签
...
%     patch(patchPointsX,patchPointsY ,figureColor,'EdgeColor',figureColor,'Parent',breakAxes);
    patch(patchPointsX, patchPointsY, [1,1,1], 'EdgeColor', figureColor, 'Parent', breakAxes);        % KYChan
%     color_add = ones(1,3)*0.85;
%     patch(patchPointsX, patchPointsY, color_add, 'EdgeColor', color_add, 'Parent', breakAxes);        % KYChan
%     patch(patchPointsX, patchPointsY, [1,1,1], 'EdgeColor', color_add, 'Parent', breakAxes);        % KYChan
%     patch(patchPointsX, patchPointsY, figureColor, 'FaceColor', 'none', 'EdgeColor', 'none', 'Parent', breakAxes);        % KYChan

    %Create A Line To Delineate the left and right edge of the patch
%     line('xData',xPoints1,'ydata',yPoints,'Parent',breakAxes,'Color',color_add,'LineWidth',mainLineWidth);
%     line('xData',xPoints2,'ydata',yPoints,'Parent',breakAxes,'Color',color_add,'LineWidth',mainLineWidth);

...
Get_FEM_Rail_P2_Through(Density, RailNodes_LSR, RailNodes_LDR, RailNodes_RSR, RailNodes_RDR, RailNodes_LCR, RailNodes_RCR, RailNodes_num_v1)
...
for k = 1:1:length(RailNodes_LSR)-1
    if ~isempty(RailNodes_LSR(k,1).Area)
        ...
        RailNodes_LSR(k,1).Mass = RailNodes_LSR(k,1).Area * Density;
    end
end
...
for k = 1:1:length(RailNodes_RDR)-1
    if ~isempty(RailNodes_RDR(k,1).Area)
        ...
        RailNodes_RDR(k,1).Mass = RailNodes_RDR(k,1).Area * Density;
    end
end
...
for k = 1:1:length(RailNodes_LCR)-1
    if ~isempty(RailNodes_LCR(k,1).Area)
        ...
        RailNodes_LCR(k,1).Mass = RailNodes_LCR(k,1).Area * Density;
    end
end
...
for k = 1:1:length(RailNodes_RCR)-1
    if ~isempty(RailNodes_RCR(k,1).Area)
        ...
        RailNodes_RCR(k,1).Mass = RailNodes_RCR(k,1).Area * Density;
    end
end

%% RSR
% figure(1); clf
% for i = 1:1:length(RailNodes_RSR)-1
%     RailNodes_RSR_loc(i,1:3) = RailNodes_RSR(i).location;
%     RailNodes_RSR_I22(i,1) = RailNodes_RSR(i).I22;
% end
% plot(RailNodes_RSR_loc(:,1),RailNodes_RSR_I22);
% grid on

% [24.4025641025641,0.750000000000000,0]
% [42.4794871794872,0.750000000000000,0]
% [53.3256410256410,0.750000000000000,0]
% 24.4025641025641~42.4794871794872, >53.3256410256410
bools_RSR = ones(length(RailNodes_RSR),1);
for i = 158+1:2:278-1
    bools_RSR(i,1) = 0;
end
for i = 350+1:2:length(RailNodes_RSR)-1
    bools_RSR(i,1) = 0;
end
...
for k = 1:1:length(RailNodes_RSR)-1
    if ~isempty(RailNodes_RSR(k,1).Area)
        ...
        RailNodes_RSR(k,1).Mass = RailNodes_RSR(k,1).Area * Density;
    end
end
...
% figure(2); clf
% k = 1;
% for i = 1:1:length(RailNodes_LDR)-1
%     if ~isempty(RailNodes_LDR(i).I22)
%         RailNodes_LDR_loc(k,1:3) = RailNodes_LDR(i).location;
%         RailNodes_LDR_I22(k,1) = RailNodes_LDR(i).I22;
%         k = k+1;
%     end
% end
% plot(RailNodes_LDR_loc(:,1),RailNodes_LDR_I22);
% grid on
...
for i = 295+1:2:length(RailNodes_LDR)-1
    bools_LDR(i,1) = 0;
end
...
for k = 1:1:length(RailNodes_LDR)-1
    if ~isempty(RailNodes_LDR(k,1).Area)
        ...
        RailNodes_LDR(k,1).Mass = RailNodes_LDR(k,1).Area * Density;
    end
end

%%
RailNodes_num_v1_temp = RailNodes_num_v1;
RailNodes_num_v1_temp(467:507,3) = RailNodes_num_v1_temp(467:507,5);

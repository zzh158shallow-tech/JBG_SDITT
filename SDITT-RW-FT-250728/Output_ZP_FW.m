%% 结果输出函数
function Output_ZP_FW(Con_FF, Con_FR, Con_RF, Con_RR, Zwy, Pjcc, Pjch, Prhxf, Dis_Rail, Vel_Rail, Acc_Rail, Profile_ProCS)
...
global ZP_FX_FF_L1 ZP_FX_FF_L2 ZP_FX_FF_R2 ZP_FX_FF_R1 ZP_FY_FF_L1 ZP_FY_FF_L2 ZP_FY_FF_R2 ZP_FY_FF_R1 ZP_FZ_FF_L1 ZP_FZ_FF_L2 ZP_FZ_FF_R2 ZP_FZ_FF_R1
...
global ZP_Con_RelVel_max_L1 ZP_Con_RelVel_max_R1 ZP_Con_RelVel_max_R2 ZP_Con_RelVel_max_R3
global ZP_Con_RelVel_L1 ZP_Con_RelVel_R1 ZP_Con_RelVel_R2 ZP_Con_RelVel_R3
global N_ConPatch_L N_ConPatch_R
...
for i1 = 1:1:Nw
    ...
    for k = 1:1:N_ConPatch_L+N_ConPatch_R
        eval(['t_',Expression_DummyRail{k},' = 1;']);
    end
    for k = 1:1:size(Con_str.Normal_Force_L,1)
        i2 =  Con_str.Normal_Force_L(k,4);
        eval(['ZP_Con_RelVel_', Expression_DummyRail{i2}, '{',num2str(i1),',t_', Expression_DummyRail{i2}, '}(1+xlcs,:) = [Con_str.Mileage, Con_str.Con_RelVel_L(k,:)];']);
        eval(['ZP_Con_RelVel_max_', Expression_DummyRail{i2}, '{',num2str(i1),',t_', Expression_DummyRail{i2}, '}(1+xlcs,:) = [Con_str.Mileage, Con_str.Con_RelVel_max_L(k,:)];']);
        eval(['t_', Expression_DummyRail{i2}, ' = t_', Expression_DummyRail{i2}, '+1;']);
    end
    for k = 1:1:size(Con_str.Normal_Force_R,1)
        i2 =  Con_str.Normal_Force_R(k,4);
        eval(['ZP_Con_RelVel_', Expression_DummyRail{i2}, '{',num2str(i1),',t_', Expression_DummyRail{i2}, '}(1+xlcs,:) = [Con_str.Mileage, Con_str.Con_RelVel_R(k,:)];']);
        eval(['ZP_Con_RelVel_max_', Expression_DummyRail{i2}, '{',num2str(i1),',t_', Expression_DummyRail{i2}, '}(1+xlcs,:) = [Con_str.Mileage, Con_str.Con_RelVel_max_R(k,:)];']);
        eval(['t_', Expression_DummyRail{i2}, ' = t_', Expression_DummyRail{i2}, '+1;']);        
    end
...
ZP_Mileage(1+xlcs,:) = {Con_FF.Mileage, Profile_ProCS.L1.FF_Profile_num, Profile_ProCS.L2.FF_Profile_num, Profile_ProCS.R1.FF_Profile_num, Profile_ProCS.R2.FF_Profile_num};

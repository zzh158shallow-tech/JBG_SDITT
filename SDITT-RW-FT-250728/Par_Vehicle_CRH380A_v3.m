...
function Par_Vehicle = Par_Vehicle_CRH380A_v3(InpPar)
...
Par_Vehicle.Ll_J = 0.750;                 % 轴箱转臂节点距转向架中心纵向距离 m
Par_Vehicle.Ll_DPz = 1.480;            % 一系垂向减振器距转向架中心纵向距离 m
...
Par_Vehicle.C1z = 0;                        % 一系垂向悬挂非线性阻尼（每轴箱）  N.s/m

Par_Vehicle.K_Jx = 13720000;           % 轴箱转臂节点纵向刚度（每轴箱）  N/m
Par_Vehicle.K_Jy = 5520000;             % 轴箱转臂节点横向刚度（每轴箱）  N/m

Par_Vehicle.K_DPz = 4.9e6;               % 一系垂向减振器接头刚度（每轴箱）  N/m
Par_Vehicle.C_DPz = 0;                       % 一系垂向减振器阻尼（每轴箱）  N.s/m（Non-Linear）
Par_Vehicle.C_DPz(1,1) = 19.6e3;                       % 一系垂向减振器阻尼（每轴箱）  N.s/m（Non-Linear）
% Par_Vehicle.C_DPz(2,1) = 19.6e3;                       % 一系垂向减振器阻尼（每轴箱）  N.s/m（Non-Linear）-Test
...
Par_Vehicle.C_Sx = 0;                         % 抗蛇行减振器阻尼（每个）  N/m（Non-Linear）
Par_Vehicle.C_Sx(1,1) = 2.4525e6;                       % 一系垂向减振器阻尼（每轴箱）  N.s/m（Non-Linear）
Par_Vehicle.C_Sx(2,1) = 49.95e3;                         % 一系垂向减振器阻尼（每轴箱）  N.s/m（Non-Linear）
Par_Vehicle.C_Sx_V0 =   0.004;                             % 一系垂向减振器阻尼（每轴箱）  N.s/m（Non-Linear）

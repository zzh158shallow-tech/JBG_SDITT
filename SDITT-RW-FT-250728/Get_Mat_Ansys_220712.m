...
%%% ModeFreq
NM_del = 0;
NM_Rail.All = 1328-NM_del;
fid = fopen('Modefile_PT-220709_FreqModal.txt');
...
fid = fopen('Modefile_PT-220709_Cons_Mode550.txt');
...
for j = 1:1:NM_Rail.All
    fid = fopen(['Modefile_PT-220709_Cons_Mode',num2str(j+NM_del),'.txt']);
    temp = textscan(fid, repmat(' %f',1,7), 'HeaderLines',1);
    fclose(fid);
    Modefile_temp = [temp{1},temp{2},temp{3},temp{4},temp{5},temp{6},temp{7}];    
    for i = 1:1:DOF_Rail.Cons
...
ModeShape.zjbg = ModeShape.Cons(p:q,:);
p = DOF_Rail_start(2,1)+1;
q = DOF_Rail_start(3,1);
ModeShape.qjbg = ModeShape.Cons(p:q,:);

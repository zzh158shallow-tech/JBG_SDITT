...
% 250717: CR400AF，考虑抗蛇形减振器*4个/BG
% 250722: CR400BF，考虑二系垂向减振器*4个/BG

function [Mlc, Klc, Clc, Clc_0] = Matrix_CR400BF_RW_230722(InpPar, Par_Vehicle)
...
Ll_SS_DPzt = Par_Vehicle.Ll_SS_DPzt;               % 二系垂向减振器与转向架中心的纵向距离 m  √
Lb_SS_DPz = Par_Vehicle.Lb_SS_DPz;            % 二系垂向减振器 横向跨距之半  m  √  (简化)
K_SS_DPz = Par_Vehicle.K_SS_DPz;                % 二系垂向减振器节点刚度  N/m
C_SS_DPz = Par_Vehicle.C_SS_DPz;                % 二系垂向减振器阻尼  N.s/m（Linear）
...
for i1 = 1:1:2
    for i2 = 1:1:2
        for i3 = 1:1:2
            pos_B = 20+5*(i1-1);
            pos_P = 35+20+4*(i1-1)+2*(i2-1)+i3;
          
            % 56~63列：z_SS_DPz (n=1~8)
            Klc(pos_P, pos_P) = Klc(pos_P, pos_P) + K_SS_DPz;
            Klc(pos_B+1, pos_P) = Klc(pos_B+1, pos_P) - K_SS_DPz;
            Klc(pos_B+3, pos_P) = Klc(pos_B+3, pos_P) - ((-1)^i3)*Lb_SS_DPz*K_SS_DPz;
            Klc(pos_B+4, pos_P) = Klc(pos_B+4, pos_P) - ((-1)^i2)*Ll_SS_DPzt*K_SS_DPz;
            
            % 21, 26列：zt (i=1~2)
            Klc(pos_P, pos_B+1) = Klc(pos_P, pos_B+1) - K_SS_DPz;
            Klc(pos_B+1, pos_B+1) = Klc(pos_B+1, pos_B+1) + K_SS_DPz;
            Klc(pos_B+3, pos_B+1) = Klc(pos_B+3, pos_B+1) + ((-1)^i3)*Lb_SS_DPz*K_SS_DPz;
            Klc(pos_B+4, pos_B+1) = Klc(pos_B+4, pos_B+1) + ((-1)^i2)*Ll_SS_DPzt*K_SS_DPz;
            
            % 23, 28列：φt (i=1~2)
            Klc(pos_P, pos_B+3) = Klc(pos_P, pos_B+3) - ((-1)^i3)*Lb_SS_DPz*K_SS_DPz;
            Klc(pos_B+1, pos_B+3) = Klc(pos_B+1, pos_B+3) + ((-1)^i3)*Lb_SS_DPz*K_SS_DPz;
            Klc(pos_B+3, pos_B+3) = Klc(pos_B+3, pos_B+3) + Lb_SS_DPz*Lb_SS_DPz*K_SS_DPz;
            Klc(pos_B+4, pos_B+3) = Klc(pos_B+4, pos_B+3) + ((-1)^i3)*Lb_SS_DPz*((-1)^i2)*Ll_SS_DPzt*K_SS_DPz;
            
            % 24, 29列：βt (i=1~2)
            Klc(pos_P, pos_B+4) = Klc(pos_P, pos_B+4) - ((-1)^i2)*Ll_SS_DPzt*K_SS_DPz;
            Klc(pos_B+1, pos_B+4) = Klc(pos_B+1, pos_B+4) + ((-1)^i2)*Ll_SS_DPzt*K_SS_DPz;
            Klc(pos_B+3, pos_B+4) = Klc(pos_B+3, pos_B+4) + ((-1)^i2)*Ll_SS_DPzt*((-1)^i3)*Lb_SS_DPz*K_SS_DPz;
            Klc(pos_B+4, pos_B+4) = Klc(pos_B+4, pos_B+4) + Ll_SS_DPzt*Ll_SS_DPzt*K_SS_DPz;
        end
    end
end
    ...
    for i2 = 1:1:2
        for i3 = 1:1:2
            ...
            if (i1==1&&i2==1) || (i1==2&&i2==2)
                Ll_SS_DPzc = Ll2+Ll_SS_DPzt;
            else
                Ll_SS_DPzc = Ll2-Ll_SS_DPzt;
            end
            
            % 31列：zc
            Clc(pos_C+1, pos_C+1) = Clc(pos_C+1, pos_C+1) + C_SS_DPz;
            Clc(pos_P, pos_C+1) = Clc(pos_P, pos_C+1) - C_SS_DPz;
            Clc(pos_C+3, pos_C+1) = Clc(pos_C+3, pos_C+1) + ((-1)^i3)*Lb_SS_DPz*C_SS_DPz;
            Clc(pos_C+4, pos_C+1) = Clc(pos_C+4, pos_C+1) + ((-1)^i1)*Ll_SS_DPzc* C_SS_DPz;
            
            % 56~63列：z_SS_DPz (n=1~8)
            Clc(pos_C+1, pos_P) = Clc(pos_C+1, pos_P) - C_SS_DPz;
            Clc(pos_P, pos_P) = Clc(pos_P, pos_P) + C_SS_DPz;
            Clc(pos_C+3, pos_P) = Clc(pos_C+3, pos_P) - ((-1)^i3)*Lb_SS_DPz*C_SS_DPz;
            Clc(pos_C+4, pos_P) = Clc(pos_C+4, pos_P) - ((-1)^i1)*Ll_SS_DPzc* C_SS_DPz;
            
            % 33列：φc
            Clc(pos_C+1, pos_C+3) = Clc(pos_C+1, pos_C+3) + ((-1)^i3)*Lb_SS_DPz*C_SS_DPz;
            Clc(pos_P, pos_C+3) = Clc(pos_P, pos_C+3) - ((-1)^i3)*Lb_SS_DPz*C_SS_DPz;
            Clc(pos_C+3, pos_C+3) = Clc(pos_C+3, pos_C+3) + Lb_SS_DPz*Lb_SS_DPz*C_SS_DPz;
            Clc(pos_C+4, pos_C+3) = Clc(pos_C+4, pos_C+3) + ((-1)^i1)*Ll_SS_DPzc*((-1)^i3)*Lb_SS_DPz*C_SS_DPz;
                       
            % 34列：βc
            Clc(pos_C+1, pos_C+4) = Clc(pos_C+1, pos_C+4) + ((-1)^i1)*Ll_SS_DPzc* C_SS_DPz;
            Clc(pos_P, pos_C+4) = Clc(pos_P, pos_C+4) - ((-1)^i1)*Ll_SS_DPzc* C_SS_DPz;
            Clc(pos_C+3, pos_C+4) = Clc(pos_C+3, pos_C+4) + ((-1)^i3)*Lb_SS_DPz*((-1)^i1)*Ll_SS_DPzc*C_SS_DPz;
            Clc(pos_C+4, pos_C+4) = Clc(pos_C+4, pos_C+4) + Ll_SS_DPzc*Ll_SS_DPzc*C_SS_DPz;            
        end
    end
end

...
% [Mass_Rail.Cons, Mass_Rail.Cons_Mapping, DOF_Rail.Cons] = Load_HBFILE('HBFILE_PT-220709_Cons_MASS');
% [Stiff_Rail.Cons, Stiff_Rail.Cons_Mapping, DOF_Rail.Cons] = Load_HBFILE('HBFILE_PT-220709_Cons_STIFF');

[Mass_Rail.Cons_Mapping, DOF_Rail.Cons] = Load_HBFILE_Mapping('HBFILE_PT-220709_SolidSlab_Cons_MASS');
[Stiff_Rail.Cons_Mapping, DOF_Rail.Cons] = Load_HBFILE_Mapping('HBFILE_PT-220709_SolidSlab_Cons_STIFF');
...
Pos_Node.zjbg = pos_Node_ori(1:717, :);
...
Pos_Node.qjbg = pos_Node_ori(717+1:end,:);
...
N_Node.Baseplate = (DOF_Rail.Cons-N_Node.Rail*DOF_Node_Rail)/DOF_Node_Slab;
...
Pos_Node.Sleeper = [(1:1:180)' Pos_Node.zjbg(bools,2)];
% bools = 1:4:N_Node.zjbg;
...
DOF_Rail.Slab = N_Node.Baseplate*DOF_Node_Slab;
...
DOF_Rail_start(:,2) = DOF_Rail.Slab;
...
len = DOF_Rail_start(end,3);
clear Mapping_temp
Mapping_temp = cell(len,3);
Mapping_temp(:,1) = num2cell(1:1:len);
for i = 1:1:N_Node.Rail
...
%%% ModeFreq
NM_del = 0;
NM_Rail.All = 1230-NM_del;
fid = fopen('Modefile_PT-220709_SolidSlab_FreqModal.txt');
...
fid = fopen('Modefile_PT-220709_SolidSlab_Mode550.txt');
...
for i = 1:1:DOF_Rail_start(end,3)
...
ModeShape.FT_Cons_All = zeros(DOF_Rail_start(end,3), NM_Rail.All);
for j = 1:1:NM_Rail.All
    fid = fopen(['Modefile_PT-220709_SolidSlab_Mode',num2str(j+NM_del),'.txt']);
    temp = textscan(fid, repmat(' %f',1,7), 'HeaderLines',1);
    fclose(fid);
    Modefile_temp = [temp{1},temp{2},temp{3},temp{4},temp{5},temp{6},temp{7}];    
%     for i = 1:1:DOF_Rail.Cons
...
ModeShape.zjbg_All = ModeShape.FT_Cons_All(p:q,:);
p = DOF_Rail_start(2,1)+1;
q = DOF_Rail_start(3,1);
ModeShape.qjbg_All = ModeShape.FT_Cons_All(p:q,:);
...
Choose_Damp_AddCons = 0;
if Choose_Damp_AddCons==1
    %%% Mass_Rail, Stiff_Rail, Damp_Rail, Pos_Node, N_Node, DOF_Rail£¨ NM_Rail, ModeFreq, ModeShape
    % [Target_Mat] = Matrix_PosTrans(Source_Mat, Source_Mapping, Target_Mapping, N_DOF_Rail)
    ...
    for i = 1:1:size(Target_Mapping,1)
        ...
        if find(abs(Pos_Node.Sleeper(:,2)-Coor(1))<=1e-10)
            if strcmp(Target_Mapping{i,3}, 'UY')

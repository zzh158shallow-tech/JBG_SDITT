...
% 230409-FW: 考虑了轮对旋转效应对 MCK 矩阵的影响

function [Mlc, Klc, Clc, Clc_0] = Matrix_Vehicle_FW_230409(InpPar, Par_Vehicle, Par_FW, DR_FW, Choose_DR)
...
Nw = InpPar.Nw;
NM_FW = InpPar.NM_FW;
Omiga = InpPar.Vlc/Par_Vehicle.R0;
ModeShape = InpPar.ModeShape;

%% 列车质量（位移变分为行，加速度为列）
DOF_FW = Nw*NM_FW;
DOF_Veh = DOF_FW + InpPar.N_RV;
Mlc = zeros(DOF_Veh, DOF_Veh);

% FW
Mlc(1:DOF_FW,1:DOF_FW) = diag(ones(DOF_FW,1),0);
...
while i1<4
    ...
    Mlc(pos+1, pos+1)=Mw;
    Mlc(pos+2, pos+2)=Mw;
    Mlc(pos+3, pos+3)=Jwx;         %轮对侧滚惯量 Jwf  kg.m^2
    Mlc(pos+4, pos+4)=Jwy;         %轮对点头惯量 Jwf  kg.m^2        
    Mlc(pos+5, pos+5)=Jwz;         %轮对摇头惯量 Jwc  kg.m^2
    i1=i1+1;
end
...
for kk = 36:1:InpPar.N_RV
    Mlc(DOF_FW+kk, DOF_FW+kk)=1e-8;
end

%% 列车刚度（位移变分为行，位移为列）
% Klc = zeros(InpPar.N_RV, InpPar.N_RV);
Klc = zeros(DOF_Veh, DOF_Veh);

% FW
for i1 = 1:1:Nw
    p1 = NM_FW*(i1-1)+1;
    p2 = NM_FW*i1;
    Klc(p1:p2,p1:p2) = Klc(p1:p2,p1:p2) + Omiga^2*Par_FW.Matrix_C;    
    for i2 = 1:1:NM_FW
        pos = (i1-1)*NM_FW+i2;
        Klc(pos,pos) = Klc(pos,pos) + (2*pi*InpPar.ModeFreq.FW(i2,2))^2;        
    end    
end
% K1 = Omiga^2*Par_FW.Matrix_C;
% K2 = diag((2*pi*InpPar.ModeFreq.FW(:,2)).^2,0);
% max(max(abs(Klc-Klc')))
...
while i1<8
    ...
    pos_B = DOF_FW+20+5*(i1g1-1);
    pos_W = DOF_FW+5*(i1g2-1);    
    ...
    for k = 1:1:NM_FW
        Klc((i1g2-1)*NM_FW+k, pos_B+1) = Klc((i1g2-1)*NM_FW+k, pos_B+1) ...
    ...
    for k = 1:1:NM_FW
        Klc((i1g2-1)*NM_FW+k, pos_W+1) = Klc((i1g2-1)*NM_FW+k, pos_W+1) ...
                                                                        +ModeShape.FW(DOF_pos_AB(3),k)*K1z;
    end
    %%% 23、28列：φti(i = 1,2)
    ...
    for k = 1:1:NM_FW
        Klc((i1g2-1)*NM_FW+k, pos_B+3) = Klc((i1g2-1)*NM_FW+k, pos_B+3) ...
                                                                        -((-1)^(i1+1))*Lb1*ModeShape.FW(DOF_pos_AB(3),k)*K1z;
    end
    %%% 24、29列：βti(i = 1,2)
    ...
    for k = 1:1:NM_FW
        Klc((i1g2-1)*NM_FW+k, pos_B+4) = Klc((i1g2-1)*NM_FW+k, pos_B+4) ...
                                                                        -((-1)^i1g2)*Ll1*ModeShape.FW(DOF_pos_AB(3),k)*K1z;
    end
    ...
    for k = 1:1:NM_FW
        Klc((i1g2-1)*NM_FW+k, pos_W+3) = Klc((i1g2-1)*NM_FW+k, pos_W+3) ...
                                                                        +((-1)^(i1+1))*Lb1*ModeShape.FW(DOF_pos_AB(3),k)*K1z;
    end
    %%% (i1g2-1)*NM+k行：δqk (k = 1,2,...,NM)——K1z
    for k = 1:1:NM_FW
        pos_FW = (i1g2-1)*NM_FW+k;
        Klc(pos_B+1, pos_FW) = Klc(pos_B+1, pos_FW)-ModeShape.FW(DOF_pos_AB(3),k)*K1z;
        Klc(pos_W+1, pos_FW) = Klc(pos_W+1, pos_FW)+ModeShape.FW(DOF_pos_AB(3),k)*K1z;
        Klc(pos_B+3, pos_FW) = Klc(pos_B+3, pos_FW)-ModeShape.FW(DOF_pos_AB(3),k)*(-1)^(i1+1)*Lb1*K1z;
        Klc(pos_W+3, pos_FW) = Klc(pos_W+3, pos_FW)+ModeShape.FW(DOF_pos_AB(3),k)*(-1)^(i1+1)*Lb1*K1z;
        Klc(pos_B+4, pos_FW) = Klc(pos_B+4, pos_FW)-ModeShape.FW(DOF_pos_AB(3),k)*(-1)^i1g2*Ll1*K1z;
        for kk = 1:1:NM_FW
            pos_FW_2 = (i1g2-1)*NM_FW+kk;
            Klc(pos_FW_2, pos_FW) = Klc(pos_FW_2, pos_FW)+ModeShape.FW(DOF_pos_AB(3),k)*ModeShape.FW(DOF_pos_AB(3),kk)*K1z;
        end
    end
    ...
    for k = 1:1:NM_FW
        Klc(pos_B+2, (i1g2-1)*NM_FW+k) = Klc(pos_B+2, (i1g2-1)*NM_FW+k) ...
    ...
    for k = 1:1:NM_FW
        Klc(pos_B+3, (i1g2-1)*NM_FW+k) = Klc(pos_B+3, (i1g2-1)*NM_FW+k) ...
                                                                       +H_tw*ModeShape.FW(DOF_pos_AB(2),k)*K1y;
    end
    %%% 25、30行：δψti(i = 1,2)     
    ...
    for k = 1:1:NM_FW
        Klc(pos_B+5, (i1g2-1)*NM_FW+k) = Klc(pos_B+5, (i1g2-1)*NM_FW+k) ...
                                                                        -((-1)^i1g4)*Ll1*ModeShape.FW(DOF_pos_AB(2),k)*K1y;
    end
    ...
    for k = 1:1:NM_FW
        Klc(pos_W+2, (i1g2-1)*NM_FW+k) = Klc(pos_W+2, (i1g2-1)*NM_FW+k) ...
                                                                        +ModeShape.FW(DOF_pos_AB(2),k)*K1y;
    end
    %%% (i1g2-1)*NM+k行：δqk (k = 1,2,...,NM)——K1y
    for k = 1:1:NM_FW
        pos_FW = (i1g2-1)*NM_FW+k;
        Klc(pos_FW,pos_B+2) = Klc(pos_FW,pos_B+2)-ModeShape.FW(DOF_pos_AB(2),k)*K1y;
        Klc(pos_FW,pos_W+2)  = Klc(pos_FW,pos_W+2)+ModeShape.FW(DOF_pos_AB(2),k)*K1y;                                          
        Klc(pos_FW,pos_B+3) = Klc(pos_FW,pos_B+3)+ModeShape.FW(DOF_pos_AB(2),k)*H_tw*K1y;
        Klc(pos_FW,pos_B+5) = Klc(pos_FW,pos_B+5)-ModeShape.FW(DOF_pos_AB(2),k)*(-1)^i1g4*Ll1*K1y;
        for kk = 1:1:NM_FW
            pos_FW_2 = (i1g2-1)*NM_FW+kk;
            Klc(pos_FW,pos_FW_2) = Klc(pos_FW,pos_FW_2)+ModeShape.FW(DOF_pos_AB(2),k)*ModeShape.FW(DOF_pos_AB(2),kk)*K1y;
        end
    end
    ...
    for k = 1:1:NM_FW
        Klc(pos_B+4,(i1g2-1)*NM_FW+k) = Klc(pos_B+4,(i1g2-1)*NM_FW+k) ...
                                                                       -ModeShape.FW(DOF_pos_AB(1),k)*H_tw*K1x;
    end
    %%% 25、30行：δψti(i = 1,2)      
    ...
    for k = 1:1:NM_FW
        Klc(pos_W+5,(i1g2-1)*NM_FW+k) = Klc(pos_W+5,(i1g2-1)*NM_FW+k) ...
                                                                         +ModeShape.FW(DOF_pos_AB(1),k)*(-1)^(i1+1+1)*Lb1*K1x;
    end
    %%% (i1g2-1)*NM+k行：δqk (k = 1,2,...,NM)——K1x
    for k = 1:1:NM_FW
        pos_FW = (i1g2-1)*NM_FW+k;
        Klc(pos_FW,pos_B+4) = Klc(pos_FW,pos_B+4)-ModeShape.FW(DOF_pos_AB(1),k)*H_tw*K1x;
        Klc(pos_FW,pos_B+5) = Klc(pos_FW,pos_B+5)-ModeShape.FW(DOF_pos_AB(1),k)*(-1)^(i1+1+1)*Lb1*K1x;
        Klc(pos_FW,pos_W+5) = Klc(pos_FW,pos_W+5)+ModeShape.FW(DOF_pos_AB(1),k)*(-1)^(i1+1+1)*Lb1*K1x;
        for kk = 1:1:NM_FW
            pos_FW_2 = (i1g2-1)*NM_FW+kk;
            Klc(pos_FW,pos_FW_2) = Klc(pos_FW,pos_FW_2)+ModeShape.FW(DOF_pos_AB(1),k)*ModeShape.FW(DOF_pos_AB(1),kk)*K1x;
        end
    end
...
Clc = zeros(DOF_Veh, DOF_Veh);

%%% Matrix G, K, C
for i1 = 1:1:Nw
    p1 = NM_FW*(i1-1)+1;
    p2 = NM_FW*i1;
    Clc(p1:p2,p1:p2) = Clc(p1:p2,p1:p2) + (-2*Omiga*Par_FW.Matrix_G);
    
    % Consider the damping ratio of WS
    if Choose_DR==1
        for i2 = 1:1:NM_FW
            p = NM_FW*(i1-1)+i2;
            Clc(p,p) = Clc(p,p) + 2*(2*pi*InpPar.ModeFreq.FW(i2,2))*DR_FW(i2,2);
        end
    end
end
% max(max(abs((Clc+Clc'))))
...
while i1<8
    ...
    for k = 1:1:NM_FW
        Clc((i1g2-1)*NM_FW+k, pos_B+1) = Clc((i1g2-1)*NM_FW+k, pos_B+1) ...
    ...
    for k = 1:1:NM_FW
        Clc((i1g2-1)*NM_FW+k, pos_W+1) = Clc((i1g2-1)*NM_FW+k, pos_W+1) ...
                                                                        +ModeShape.FW(DOF_pos_AB(3),k)*C1z;
    end
    %%% 23、28列：φti(i = 1,2)
    ...
    for k = 1:1:NM_FW
        Clc((i1g2-1)*NM_FW+k, pos_B+3) = Clc((i1g2-1)*NM_FW+k, pos_B+3) ...
                                                                        -((-1)^(i1+1))*Lb1*ModeShape.FW(DOF_pos_AB(3),k)*C1z;
    end
    %%% 24、29列：βti(i = 1,2)
    ...
    for k = 1:1:NM_FW
        Clc((i1g2-1)*NM_FW+k, pos_B+4) = Clc((i1g2-1)*NM_FW+k, pos_B+4) ...
                                                                        -((-1)^i1g2)*Ll1*ModeShape.FW(DOF_pos_AB(3),k)*C1z;
    end
    %%% 3、8、13、18列：φwm(m=1,2,3,4)
    ...
    for k = 1:1:NM_FW
        Clc((i1g2-1)*NM_FW+k, pos_W+3) = Clc((i1g2-1)*NM_FW+k, pos_W+3) ...
                                                                        +((-1)^(i1+1))*Lb1*ModeShape.FW(DOF_pos_AB(3),k)*C1z;
    end
    %%% (i1g2-1)*NM+k行：δqk (k = 1,2,...,NM)——K1z
    for k = 1:1:NM_FW
        pos_FW = (i1g2-1)*NM_FW+k;
        Clc(pos_B+1, pos_FW) = Clc(pos_B+1, pos_FW)-ModeShape.FW(DOF_pos_AB(3),k)*C1z;
        Clc(pos_W+1, pos_FW) = Clc(pos_W+1, pos_FW)+ModeShape.FW(DOF_pos_AB(3),k)*C1z;
        Clc(pos_B+3, pos_FW) = Clc(pos_B+3, pos_FW)-ModeShape.FW(DOF_pos_AB(3),k)*(-1)^(i1+1)*Lb1*C1z;
        Clc(pos_W+3, pos_FW) = Clc(pos_W+3, pos_FW)+ModeShape.FW(DOF_pos_AB(3),k)*(-1)^(i1+1)*Lb1*C1z;
        Clc(pos_B+4, pos_FW) = Clc(pos_B+4, pos_FW)-ModeShape.FW(DOF_pos_AB(3),k)*(-1)^i1g2*Ll1*C1z;
        for kk = 1:1:NM_FW
            pos_FW_2 = (i1g2-1)*NM_FW+kk;
            Clc(pos_FW_2, pos_FW) = Clc(pos_FW_2, pos_FW)+ModeShape.FW(DOF_pos_AB(3),k)*ModeShape.FW(DOF_pos_AB(3),kk)*C1z;
        end
    end
    ...
    for k = 1:1:NM_FW
        Clc(pos_B+2, (i1g2-1)*NM_FW+k) = Clc(pos_B+2, (i1g2-1)*NM_FW+k) ...
    ...
    for k = 1:1:NM_FW
        Clc(pos_B+3, (i1g2-1)*NM_FW+k) = Clc(pos_B+3, (i1g2-1)*NM_FW+k) ...
                                                                       +H_tw*ModeShape.FW(DOF_pos_AB(2),k)*C1y;
    end
    %%% 25、30行：δψti(i = 1,2)     
    ...
    for k = 1:1:NM_FW
        Clc(pos_B+5, (i1g2-1)*NM_FW+k) = Clc(pos_B+5, (i1g2-1)*NM_FW+k) ...
                                                                        -((-1)^i1g4)*Ll1*ModeShape.FW(DOF_pos_AB(2),k)*C1y;
    end
    %%% 2、7、12、17行：δywm(m = 1,2,3,4)      
    ...
    for k = 1:1:NM_FW
        Clc(pos_W+2, (i1g2-1)*NM_FW+k) = Clc(pos_W+2, (i1g2-1)*NM_FW+k) ...
                                                                        +ModeShape.FW(DOF_pos_AB(2),k)*C1y;
    end
    %%% (i1g2-1)*NM+k行：δqk (k = 1,2,...,NM)——K1y
    for k = 1:1:NM_FW
        pos_FW = (i1g2-1)*NM_FW+k;
        Clc(pos_FW,pos_B+2) = Clc(pos_FW,pos_B+2)-ModeShape.FW(DOF_pos_AB(2),k)*C1y;
        Clc(pos_FW,pos_W+2)  = Clc(pos_FW,pos_W+2)+ModeShape.FW(DOF_pos_AB(2),k)*C1y;                                          
        Clc(pos_FW,pos_B+3) = Clc(pos_FW,pos_B+3)+ModeShape.FW(DOF_pos_AB(2),k)*H_tw*C1y;
        Clc(pos_FW,pos_B+5) = Clc(pos_FW,pos_B+5)-ModeShape.FW(DOF_pos_AB(2),k)*(-1)^i1g4*Ll1*C1y;
        for kk = 1:1:NM_FW
            pos_FW_2 = (i1g2-1)*NM_FW+kk;
            Clc(pos_FW,pos_FW_2) = Clc(pos_FW,pos_FW_2)+ModeShape.FW(DOF_pos_AB(2),k)*ModeShape.FW(DOF_pos_AB(2),kk)*C1y;
        end
    end
    ...
    for k = 1:1:NM_FW
        Clc(pos_B+4,(i1g2-1)*NM_FW+k) = Clc(pos_B+4,(i1g2-1)*NM_FW+k) ...
                                                                       -ModeShape.FW(DOF_pos_AB(1),k)*H_tw*C1x;
    end
    %%% 25、30行：δψti(i = 1,2)      
    ...
    for k = 1:1:NM_FW
        Clc(pos_W+5,(i1g2-1)*NM_FW+k) = Clc(pos_W+5,(i1g2-1)*NM_FW+k) ...
                                                                         +ModeShape.FW(DOF_pos_AB(1),k)*(-1)^(i1+1+1)*Lb1*C1x;
    end
    %%% (i1g2-1)*NM+k行：δqk (k = 1,2,...,NM)——K1x
    for k = 1:1:NM_FW
        pos_FW = (i1g2-1)*NM_FW+k;
        Clc(pos_FW,pos_B+4) = Clc(pos_FW,pos_B+4)-ModeShape.FW(DOF_pos_AB(1),k)*H_tw*C1x;
        Clc(pos_FW,pos_B+5) = Clc(pos_FW,pos_B+5)-ModeShape.FW(DOF_pos_AB(1),k)*(-1)^(i1+1+1)*Lb1*C1x;
        Clc(pos_FW,pos_W+5) = Clc(pos_FW,pos_W+5)+ModeShape.FW(DOF_pos_AB(1),k)*(-1)^(i1+1+1)*Lb1*C1x;
        for kk = 1:1:NM_FW
            pos_FW_2 = (i1g2-1)*NM_FW+kk;
            Clc(pos_FW,pos_FW_2) = Clc(pos_FW,pos_FW_2)+ModeShape.FW(DOF_pos_AB(1),k)*ModeShape.FW(DOF_pos_AB(1),kk)*C1x;
        end
    end
...
for i1 = 1:1:2
    for i2 = 1:1:2
        for i3 = 1:1:2
            pos_W = DOF_FW+10*(i1-1)+5*(i2-1);
            pos_B = DOF_FW+20+5*(i1-1);
            pos_FW = NM_FW*(2*(i1-1)+i2-1);
            DOF_pos_AB = Par_FW.DOF_pos.(['AxleBox_', InpPar.Type_Side{i3}]);
            ...
            for k = 1:1:NM_FW
                Klc(pos_FW+k, pos_B+4) = Klc(pos_FW+k, pos_B+4)-ModeShape.FW(DOF_pos_AB(1),k)*H_tJ*K_Jx;
            end
            % Col 25, 30: ψt
            ...
            for k = 1:1:NM_FW
                Klc(pos_FW+k, pos_B+5) = Klc(pos_FW+k, pos_B+5)-ModeShape.FW(DOF_pos_AB(1),k)*((-1)^(i3+1))*Lb_J*K_Jx;
            end
            % Col 4, 9, 14, 19: βw
            ...
            for k = 1:1:NM_FW
                Klc(pos_FW+k, pos_W+4) = Klc(pos_FW+k, pos_W+4)-ModeShape.FW(DOF_pos_AB(1),k)*H_Jw*K_Jx;
            end
            % Col 5, 10, 15, 20: ψw
            ...
            for k = 1:1:NM_FW
                Klc(pos_FW+k, pos_W+5) = Klc(pos_FW+k, pos_W+5)+ModeShape.FW(DOF_pos_AB(1),k)*((-1)^(i3+1))*Lb_J*K_Jx;
            end
            %%% NM_FW*(2*(i1-1)+i2-1)+k列：qk (k = 1,2,...,NM)——KJx
            for k = 1:1:NM_FW
                Klc(pos_B+4, pos_FW+k) = Klc(pos_B+4, pos_FW+k)-ModeShape.FW(DOF_pos_AB(1),k)*H_tJ*K_Jx;
                Klc(pos_B+5, pos_FW+k) = Klc(pos_B+5, pos_FW+k)-ModeShape.FW(DOF_pos_AB(1),k)*((-1)^(i3+1))*Lb_J*K_Jx;
                Klc(pos_W+4, pos_FW+k) = Klc(pos_W+4, pos_FW+k)-ModeShape.FW(DOF_pos_AB(1),k)*H_Jw*K_Jx;
                Klc(pos_W+5, pos_FW+k) = Klc(pos_W+5, pos_FW+k)+ModeShape.FW(DOF_pos_AB(1),k)*((-1)^(i3+1))*Lb_J*K_Jx;
                for kk = 1:1:NM_FW
                    Klc(pos_FW+kk,pos_FW+k) = Klc(pos_FW+kk,pos_FW+k)+ModeShape.FW(DOF_pos_AB(1),k)*ModeShape.FW(DOF_pos_AB(1),kk)*K_Jx;
                end
            end
            ...
            for k = 1:1:NM_FW
                Klc(pos_FW+k, pos_B+2) = Klc(pos_FW+k, pos_B+2)-ModeShape.FW(DOF_pos_AB(2),k)*K_Jy;
            end
            % Col 23, 28: φt
            ...
            for k = 1:1:NM_FW
                Klc(pos_FW+k, pos_B+3) = Klc(pos_FW+k, pos_B+3) + H_tJ*ModeShape.FW(DOF_pos_AB(2),k)*K_Jy;
            end
            % Col 25, 30: ψt
            ...
            for k = 1:1:NM_FW
                Klc(pos_FW+k, pos_B+5) = Klc(pos_FW+k, pos_B+5) - ((-1)^(i2+1))*Ll_Jt*ModeShape.FW(DOF_pos_AB(2),k)*K_Jy;
            end            
            % Col 2, 7, 12, 17: yw
            ...
            for k = 1:1:NM_FW
                Klc(pos_FW+k, pos_W+2) = Klc(pos_FW+k, pos_W+2)+ModeShape.FW(DOF_pos_AB(2),k)*K_Jy;
            end
            % Col 3, 8, 13, 18: φw
            ...
            for k = 1:1:NM_FW
                Klc(pos_FW+k, pos_W+3) = Klc(pos_FW+k, pos_W+3) + H_Jw*ModeShape.FW(DOF_pos_AB(2),k)*K_Jy;
            end           
            % Col 5, 10, 15, 20: ψw
            ...
            for k = 1:1:NM_FW
                Klc(pos_FW+k, pos_W+5) = Klc(pos_FW+k, pos_W+5) + ((-1)^(i2+1))*Ll_Jw*ModeShape.FW(DOF_pos_AB(2),k)*K_Jy;
            end
            %%% NM_FW*(2*(i1-1)+i2-1)+k列：qk (k = 1,2,...,NM)——KJy
            for k = 1:1:NM_FW
                Klc(pos_B+2, pos_FW+k) = Klc(pos_B+2, pos_FW+k) - ModeShape.FW(DOF_pos_AB(2),k)*K_Jy;
                Klc(pos_B+3, pos_FW+k) = Klc(pos_B+3, pos_FW+k) + H_tJ*ModeShape.FW(DOF_pos_AB(2),k)*K_Jy;                
                Klc(pos_B+5, pos_FW+k) = Klc(pos_B+5, pos_FW+k) - ((-1)^(i2+1))*Ll_Jt*ModeShape.FW(DOF_pos_AB(2),k)*K_Jy;
                Klc(pos_W+2, pos_FW+k) = Klc(pos_W+2, pos_FW+k) + ModeShape.FW(DOF_pos_AB(2),k)*K_Jy;
                Klc(pos_W+3, pos_FW+k) = Klc(pos_W+3, pos_FW+k) + H_Jw*ModeShape.FW(DOF_pos_AB(2),k)*K_Jy;
                Klc(pos_W+5, pos_FW+k) = Klc(pos_W+5, pos_FW+k) + ((-1)^(i2+1))*Ll_Jw*ModeShape.FW(DOF_pos_AB(2),k)*K_Jy;
                for kk = 1:1:NM_FW
                    Klc(pos_FW+kk,pos_FW+k) = Klc(pos_FW+kk,pos_FW+k)+ModeShape.FW(DOF_pos_AB(2),k)*ModeShape.FW(DOF_pos_AB(2),kk)*K_Jy;
                end
            end
...
for i1 = 1:1:2
    for i2 = 1:1:2
        for i3 = 1:1:2
            pos_P = DOF_FW+35+4*(i1-1)+2*(i2-1)+i3;
            pos_W = DOF_FW+10*(i1-1)+5*(i2-1);
            pos_FW = NM_FW*(2*(i1-1)+i2-1);
            DOF_pos_AB = Par_FW.DOF_pos.(['AxleBox_', InpPar.Type_Side{i3}]);
            ...
            for k = 1:1:NM_FW
                Klc(pos_FW+k, pos_P) = Klc(pos_FW+k, pos_P)-ModeShape.FW(DOF_pos_AB(3),k)*K_DPz;
            end
            ...
            for k = 1:1:NM_FW
                Klc(pos_FW+k, pos_W+1) = Klc(pos_FW+k, pos_W+1)+ModeShape.FW(DOF_pos_AB(3),k)*K_DPz;
            end
            ...
            for k = 1:1:NM_FW
                Klc(pos_FW+k, pos_W+3) = Klc(pos_FW+k, pos_W+3) + ((-1)^i3)*Lb_DPz*ModeShape.FW(DOF_pos_AB(3),k)*K_DPz;
            end
            ...
            for k = 1:1:NM_FW
                Klc(pos_FW+k, pos_W+4) = Klc(pos_FW+k, pos_W+4) + ((-1)^i2)*Ll_DPzw*ModeShape.FW(DOF_pos_AB(3),k)*K_DPz;
            end
            
            %%% NM_FW*(2*(i1-1)+i2-1)+k列：qk (k = 1,2,...,NM)——K_DPz
            for k = 1:1:NM_FW
                Klc(pos_P, pos_FW+k) = Klc(pos_P, pos_FW+k) - ModeShape.FW(DOF_pos_AB(3),k)*K_DPz;
                Klc(pos_W+1, pos_FW+k) = Klc(pos_W+1, pos_FW+k) + ModeShape.FW(DOF_pos_AB(3),k)*K_DPz;
                Klc(pos_W+3, pos_FW+k) = Klc(pos_W+3, pos_FW+k) + ((-1)^i3)*Lb_DPz*ModeShape.FW(DOF_pos_AB(3),k)*K_DPz;
                Klc(pos_W+4, pos_FW+k) = Klc(pos_W+4, pos_FW+k) + ((-1)^i2)*Ll_DPzw*ModeShape.FW(DOF_pos_AB(3),k)*K_DPz;                
                for kk = 1:1:NM_FW
                    Klc(pos_FW+kk, pos_FW+k) = Klc(pos_FW+kk, pos_FW+k) + ModeShape.FW(DOF_pos_AB(3),k)*ModeShape.FW(DOF_pos_AB(3),kk)*K_DPz;
                end
            end
...
% max(max(abs(Klc(1:DOF_FW,1:DOF_FW))))
% max(max(abs(Clc(1:DOF_FW,1:DOF_FW))))

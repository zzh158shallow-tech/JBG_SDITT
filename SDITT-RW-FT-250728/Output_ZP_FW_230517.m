...
function [ZP_Dyn, ZP_Con] = Output_ZP_FW_230517(InpPar, ZP_Dyn, ZP_Con, xlcs, Par_Track, Con_WS, Zwy, Pjcc, Pjch, Prhxf, Dis_Rail, Vel_Rail, Acc_Rail, RailPro_ProCS, drtaT, i_drtaT, times_drtaT, T, DamperNL)
...
%             ZP_Con.Wheel_3_RW.(Tk){i1,Target_tt}(1+xlcs,:) = [Con_str.Mileage, Con_str.Con_wheel_3_RW.(T2)(k,:)];
%             if InpPar.NM_FW>0
                ZP_Con.Wheel_2_RW.(Tk){i1,Target_tt}(1+xlcs,:) = [Con_str.Mileage, Con_str.Con_wheel_2_RW.(T2)(k,1:end-1), Con_str.elastic_permeability_Unit.(T2)(k)];
%                 ZP_Con.Rail_2_RW.(Tk){i1,Target_tt}(1+xlcs,:) = [Con_str.Mileage, Con_str.Con_rail_2_RW.(T2)(k,:)];
%             end
            ...
            if strcmp(InpPar.Type_Normal, 'STRIPES&ConDamp')
                if ~isempty(Con_str.Epsilon.(T2))
                    ...
                    % K_Stiff_j, NorGap_Con_j(:,2), NF_j{i,1}
                    ZP_Con.STRIPES_Stiff_Pen.(Tk){i1,Target_tt}{1+xlcs,2} = Con_str.Con_STRIPES.(T2){k,4};  % Con_STRIPES_oup{i,4} = [K_Stiff_j, NorGap_Con_j(:,2), NF_j{i,1}];
                    % Track C.S.中，钢轨上条带坐标
                    ZP_Con.STRIPES_Stiff_Pen.(Tk){i1,Target_tt}{1+xlcs,3} = Con_str.Con_STRIPES.(T2){k,2};  % Con_STRIPES_oup{i,2} = Con_rail_1_Patch;
                end
            end
